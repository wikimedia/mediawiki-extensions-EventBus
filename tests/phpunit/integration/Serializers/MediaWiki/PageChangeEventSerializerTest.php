...
class PageChangeEventSerializerTest extends MediaWikiIntegrationTestCase {
	...
	public function setUp(): void {
		...
		$this->pageChangeEventSerializer = new PageChangeEventSerializer(
			...
			$this->revisionStore,
			(int)$services->getMainConfig()->get(
				'EventBusPageChangeRevertedRevIdsMax',
				15
			),
		);
	...
	public function testCreatePageChangeEditEventWithRevertMetadata() {
		$wikiPage0 = $this->getExistingTestPage(
			Title::newFromText( 'MyPageRevertMeta', $this->getDefaultWikitextNS() )
		);
		...
		$expected = $this->createExpectedPageChangeEvent(
			...
			[
				...
				'changelog_kind' => 'update',
				'prior_state' => [
					'revision' => $this->revisionEntitySerializer->toArray( $parentRevision ),
				],
				'revision' => [
					'revert' => [
						'is_exact' => true,
						...
						'rev_reverted_ids' => [ $currentRevision->getId() ],
					],
				],
			]
		);
	...
	public function testCreatePageChangeEditEventWithRevertMetadataWithoutOriginalRevisionId() {
		$wikiPage0 = $this->getExistingTestPage(
			Title::newFromText( 'MyPageRevertMetaNoOriginal', $this->getDefaultWikitextNS() )
		);
	 ...
	 * @covers ::toEditEvent
	 */
	public function testCreatePageChangeEditEventOmitsRevertedIdsWhenEventBusRevertedRevIdsMaxIsZero() {
		$wikiPage0 = $this->getExistingTestPage(
			Title::newFromText( 'MyPageRevertTrunc', $this->getDefaultWikitextNS() )
		);
		...
		$pageChangeEventSerializer = new PageChangeEventSerializer(
			$this->eventSerializer,
			$this->pageEntitySerializer,
			$this->userEntitySerializer,
			$this->revisionEntitySerializer,
		...
		$actual = $pageChangeEventSerializer->toEditEvent(
			self::MOCK_STREAM_NAME,
			$wikiPage0,
			$this->userFactory->newFromUserIdentity(
				$wikiPage0->getRevisionRecord()->getUser()
